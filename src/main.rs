...
use calc::series_xy;
use fltk::{app::{self, awake}, enums::{self}, frame::{self, Frame}, prelude::*, window::{DoubleWindow, Window}};
...
pub fn draw_and_set_callbacks_on_ui(
    ...
    app_sender: &app::Sender<ChannelMessage>,
) {
    ...
    let _animator = thread::spawn(move || loop {
        if let Ok(msg) = t_r.recv() {
            match msg {
                ...
                ButtonAnimation::DC(dc) => distribute_cards_on_ui(
                    dc,
                    &mut bottom_cards,
                    &mut top_cards,
                    &mut cards_on_decs,
